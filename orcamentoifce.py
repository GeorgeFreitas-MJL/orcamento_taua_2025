...
# Função para carregar a planilha
@st.cache_data(ttl=0)  # Limpa o cache a cada atualização
def carregar_planilha(caminho):
    try:
...
planilhas_dfs = {nome: carregar_planilha(caminho) for nome, caminho in files.items()}
...
selected_sheet = st.selectbox('🖱️ Escolha a AÇÃO para visualização do gráfico ⤵️', list(planilhas_dfs.keys()))

if selected_sheet:
    selected_df = planilhas_dfs[selected_sheet]

    # Filtra valores válidos para PAGAMENTO REALIZADO
    if 'PAGAMENTO REALIZADO' in selected_df.columns:
        selected_df = selected_df[selected_df['PAGAMENTO REALIZADO'] > 0]

        # Calcular Percentual e criar rótulos
        selected_df['Percentual'] = selected_df['PAGAMENTO REALIZADO'] / selected_df['PAGAMENTO REALIZADO'].sum() * 100

        # Criação dos rótulos
        selected_df['Label'] = selected_df.apply(lambda row: f'{selected_sheet} - R$ {row['PAGAMENTO REALIZADO']:.2f} ({row['Percentual']:.2f}%)', axis=1)

        # Geração do gráfico
        cores_personalizadas = px.colors.qualitative.Set3 + px.colors.qualitative.Pastel + px.colors.qualitative.Safe
        fig = px.pie(selected_df, values='Percentual', names='Label', hole=0.3, color_discrete_sequence=cores_personalizadas)

        # Atualização do subtítulo
        st.subheader(f'Distribuição percentual de valores pagos no gráfico abaixo:')
        st.plotly_chart(fig, use_container_width=True)
    else:
        st.warning('A coluna PAGAMENTO REALIZADO não foi encontrada na planilha selecionada.')
...
with tab2:
    st.markdown('⤵️ Visualização da Planilha Completa')
    st.dataframe(df)
